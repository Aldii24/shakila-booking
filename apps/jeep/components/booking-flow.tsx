      ...
      {result && pack ? (
        <article className="result-card">
          <div>
            ...
            <h3>{pack.name}</h3>
            <p>
              {result.availableQuantity} Jeep tersedia pada slot ini · kapasitas {pack.capacityPerUnit} tamu/Jeep · data demo
            </p>
...
export function PaymentPage({ bookingCode }: { bookingCode: string }) {
  ...
  return <div className="flow-grid manual-transfer-flow"><div className="panel"><p className="kicker">TRANSFER BANK MANUAL</p><h2>{status.bookingCode}</h2><p>{status.productName} · {status.startDate} · {status.departureTime?formatDepartureTime(status.departureTime):"—"}</p><p className="payment-recovery-note"><ShieldCheck aria-hidden="true"/> Halaman ini aman untuk di-refresh. Akses booking dan nominal transfer disimpan selama maksimal 24 jam di perangkat ini. Demi privasi, foto bukti yang belum dikirim harus dipilih kembali.</p>
    ...
    {!approved&&!expired?<><div className="countdown"><Clock3/> {countdown}</div><div className="bank-instructions"><p className="kicker">INSTRUKSI TRANSFER</p><h3>{process.env.NEXT_PUBLIC_BANK_NAME??"Rekening demo Shakila Group"}</h3><p>{process.env.NEXT_PUBLIC_BANK_ACCOUNT??"Nomor rekening akan dikonfigurasi setelah data resmi klien diterima."}</p><strong>{process.env.NEXT_PUBLIC_BANK_HOLDER??"Shakila Group"}</strong><small>Gunakan kode booking sebagai berita transfer.</small></div>{!pending?<div className="proof-upload"><Label>Nominal yang ditransfer<Input type="number" min={status.requiredDpAmount} max={status.totalAmount} value={effectiveAmount} onChange={(event)=>setClaimedAmount(Number(event.target.value))}/></Label><div className="proof-field"><span>Bukti pembayaran</span><ProofFilePicker file={file} onFileChange={selectProof}/></div>{preview?<div className="proof-preview" role="img" aria-label="Pratinjau bukti pembayaran" style={{backgroundImage:`url(${preview})`}}/>:null}<Button className="button" disabled={busy||!file||effectiveAmount<status.requiredDpAmount} onClick={()=>void submitProof()}>{busy?"Mengunggah bukti...":"Kirim bukti pembayaran"}</Button></div>:null}</>:null}
