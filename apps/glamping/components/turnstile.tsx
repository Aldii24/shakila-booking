...
export function Turnstile({onToken}:{onToken:(token:string)=>void}){
  ...
  return <div><div ref={ref}/>{demoDisabled?<p className="status-note">Proteksi bot dinonaktifkan secara eksplisit untuk mode demo.</p>:!process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY&&process.env.NEXT_PUBLIC_TURNSTILE_LOCAL_BYPASS!=="true"?<p className="status-note error-note">Turnstile belum dikonfigurasi untuk lingkungan ini.</p>:null}</div>
}
