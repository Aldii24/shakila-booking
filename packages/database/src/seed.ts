...
(${glampingId},'GLP','glamping','Shakila Glamping','ACCOMMODATION','reservasi@shakilagroup.demo','+6281230001000','Sukapura, Probolinggo, Jawa Timur'),
(${jeepId},'JEP','jeep','Shakila Jeep Tour','ACTIVITY','tour@shakilagroup.demo','+6281230002000','Sukapura, Probolinggo, Jawa Timur')
...
(${glampingId},30,30,'14:00','11:00','reservasi@shakilagroup.demo','+6281230001000'),(${jeepId},30,30,null,null,'tour@shakilagroup.demo','+6281230002000')
on conflict(business_id) do update set dp_percentage=30,booking_hold_minutes=30,updated_at=now();
...
select md5('glamp-booking-'||i)::uuid,
 ...
 case when i between 22 and 24 then now()+interval '30 minutes' else null end,
...
await client.begin(async transaction => {
  ...
  await transaction.unsafe(`
    ...
    insert into bookings (id,booking_code,business_id,customer_id,booking_type,status,payment_status,customer_name,customer_email,customer_whatsapp,customer_email_normalized,customer_whatsapp_normalized,guest_count,quantity,subtotal_amount,total_amount,dp_percentage,required_dp_amount,verified_paid_amount,remaining_amount,expires_at,confirmed_at,cancelled_at,checked_in_at,checked_out_at,completed_at,created_at,updated_at)
    select md5('jeep-booking-'||i)::uuid,'JEP-'||to_char(current_date,'YYMMDD')||'-9'||lpad(i::text,5,'0'),${jeepId},c.id,'JEEP',case when i<=6 then 'COMPLETED'::booking_status when i=7 then 'CHECKED_OUT' when i=8 then 'CHECKED_IN' when i<=15 then 'CONFIRMED' when i<=17 then 'WAITING_PAYMENT' when i=18 then 'EXPIRED' else 'CANCELLED' end,case when i<=15 then case when i%3=0 then 'PAID'::payment_status else 'PARTIALLY_PAID' end else 'UNPAID' end,c.full_name,c.email,c.whatsapp,c.email_normalized,c.whatsapp_normalized,2,1,case when i%2=0 then 950000 else 750000 end,case when i%2=0 then 950000 else 750000 end,30,case when i%2=0 then 285000 else 225000 end,case when i<=15 then case when i%3=0 then case when i%2=0 then 950000 else 750000 end else case when i%2=0 then 285000 else 225000 end end else 0 end,case when i<=15 and i%3=0 then 0 when i<=15 then case when i%2=0 then 665000 else 525000 end else case when i%2=0 then 950000 else 750000 end end,case when i between 16 and 17 then now()+interval '30 minutes' else null end,case when i<=15 then now()-interval '2 days' else null end,case when i=19 then now()-interval '1 day' else null end,case when i<=8 then now()-interval '4 hours' else null end,case when i<=7 then now()-interval '1 hour' else null end,case when i<=6 then now() else null end,now()-interval '8 days',now() from generate_series(1,19)i join customers c on c.id=md5('customer-'||(1+(i+10)%30))::uuid;
