...
export function JeepAvailabilityCalendar({ initial }: { initial: { tourDate?: string; guestCount?: string } }) {
  ...
  return <section className="jeep-availability-calendar">
    <div className="jeep-calendar-head"><div><p className="kicker">KETERSEDIAAN ARMADA LANGSUNG · STOK 8 UNIT DATA DEMO</p><h2>{new Intl.DateTimeFormat("id-ID", { month: "long", year: "numeric", timeZone: "UTC" }).format(month)}</h2></div><div><Button variant="ghost" size="icon" aria-label="Bulan sebelumnya" onClick={() => setMonth(new Date(Date.UTC(month.getUTCFullYear(), month.getUTCMonth() - 1, 1)))}><ChevronLeft /></Button><Button variant="ghost" onClick={() => setMonth(new Date(`${today().slice(0, 7)}-01T00:00:00Z`))}>HARI INI</Button><Button variant="ghost" size="icon" aria-label="Bulan berikutnya" onClick={() => setMonth(new Date(Date.UTC(month.getUTCFullYear(), month.getUTCMonth() + 1, 1)))}><ChevronRight /></Button></div></div>
