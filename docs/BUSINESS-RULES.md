...
> **Klarifikasi client — 9 September 2026 (otoritatif):** Booking accommodation, termasuk booking manual Admin untuk Glamping/Homestay, mengikuti `CONFIRMED → PAID → CHECKED_IN → CHECKED_OUT`; Check-In tersedia mulai pukul 13.00 WIB pada tanggal reservasi dan tetap tersedia setelah waktu tersebut. Booking Jeep tidak menggunakan Check-In/Check-Out. Setelah `CONFIRMED` dan `PAID`, Admin memakai tindakan `Selesai` untuk mengubah booking Jeep menjadi `COMPLETED` dan langsung melepas alokasi armada pada hari yang sama. `WAITING_PAYMENT`, `CONFIRMED`, dan accommodation `CHECKED_IN` menahan unit; `CANCELLED`, `EXPIRED`, `CHECKED_OUT`, dan Jeep `COMPLETED` melepasnya.
