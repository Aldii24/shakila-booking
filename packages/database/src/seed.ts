...
const clientJeepSql = clientJeepPackages.map((item, index) => {
  ...
  return `insert into jeep_packages (id,business_id,slug,name,description,price_per_unit,capacity_per_unit,routes,facilities,media_key,is_demo_data,sort_order) values (${packageId},${jeepId},'${item.slug}','${item.name}','${description}',${item.price},1,'${JSON.stringify(item.routes)}'::jsonb,'["Jeep wisata","Pengemudi lokal"]'::jsonb,'jeep/katalog-jeep.jpg',false,${index + 1}) on conflict(id) do update set slug=excluded.slug,name=excluded.name,description=excluded.description,price_per_unit=excluded.price_per_unit,capacity_per_unit=excluded.capacity_per_unit,routes=excluded.routes,facilities=excluded.facilities,media_key=excluded.media_key,is_demo_data=excluded.is_demo_data,is_active=true,sort_order=excluded.sort_order,updated_at=now();
insert into jeep_departure_slots (id,business_id,jeep_package_id,name,departure_time,is_demo_data) values (${uuid(`client-jeep-${item.slug}-schedule`)},${jeepId},${packageId},'Jadwal Keberangkatan',null,false) on conflict(id) do update set name=excluded.name,departure_time=excluded.departure_time,is_demo_data=false,is_active=true,updated_at=now();`;
}).join("\n");
...
(${homestayTwinId},${glampingId},'homestay-twin-bed','Twin Bed','HOMESTAY','Kamar Homestay Twin Bed di kawasan Nepal van Java.',400000,1,null,'["Kamar mandi dalam","Air hangat","TV","Wi-Fi gratis","Minuman selamat datang","Peralatan mandi","Menu makan","Teras atap"]'::jsonb,'akomodasi/homestay-twin-bed.jpg',false,50)
...
await client.begin(async transaction => {
  ...
  await transaction.unsafe(`
    ...
    insert into bookings (id,booking_code,business_id,customer_id,booking_type,status,payment_status,customer_name,customer_email,customer_whatsapp,customer_email_normalized,customer_whatsapp_normalized,guest_count,quantity,subtotal_amount,total_amount,dp_percentage,required_dp_amount,verified_paid_amount,remaining_amount,expires_at,confirmed_at,cancelled_at,checked_in_at,checked_out_at,completed_at,created_at,updated_at)
    select md5('jeep-booking-'||i)::uuid,'JEP-'||to_char(current_date,'YYMMDD')||'-9'||lpad(i::text,5,'0'),${jeepId},c.id,'JEEP',case when i<=8 then 'COMPLETED'::booking_status when i<=15 then 'CONFIRMED' when i<=17 then 'WAITING_PAYMENT' when i=18 then 'EXPIRED' else 'CANCELLED' end,case when i<=15 then case when i%3=0 then 'PAID'::payment_status else 'PARTIALLY_PAID' end else 'UNPAID' end,c.full_name,c.email,c.whatsapp,c.email_normalized,c.whatsapp_normalized,2,1,case when i%2=0 then 950000 else 750000 end,case when i%2=0 then 950000 else 750000 end,50,case when i%2=0 then 475000 else 375000 end,case when i<=15 then case when i%3=0 then case when i%2=0 then 950000 else 750000 end else case when i%2=0 then 475000 else 375000 end end else 0 end,case when i<=15 and i%3=0 then 0 when i<=15 then case when i%2=0 then 475000 else 375000 end else case when i%2=0 then 950000 else 750000 end end,case when i between 16 and 17 then now()+interval '12 hours' else null end,case when i<=15 then now()-interval '2 days' else null end,case when i=19 then now()-interval '1 day' else null end,null,null,case when i<=8 then now() else null end,now()-interval '8 days',now() from generate_series(1,19)i join customers c on c.id=md5('customer-'||(1+(i+10)%30))::uuid;
    ...
    insert into jeep_unit_reservations (booking_id,jeep_unit_id,departure_slot_id,tour_date,state,released_at) select b.id,md5('jeep-unit-'||(1+(n-1)%12))::uuid,j.departure_slot_id,j.tour_date,case when b.status in('CONFIRMED','WAITING_PAYMENT') then case when b.status='CONFIRMED' then 'CONFIRMED'::reservation_state else 'HELD' end else 'RELEASED' end,case when b.status in('COMPLETED','EXPIRED','CANCELLED') then now() else null end from bookings b join jeep_booking_details j on j.booking_id=b.id cross join lateral(select right(b.booking_code,5)::int n)x where b.booking_code like 'JEP-%-9%';
